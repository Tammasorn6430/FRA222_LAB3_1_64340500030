  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  if(HAL_GetTick() >= timestamp){
		  ...
		  MotorReadRPM = IC_Calc_Period() * 60;
	  }
  }
  /* USER CODE END 3 */
}
...
float IC_Calc_Period()
{
    ...
    uint32_t i = (lastValidDMAPointer + IC_BUFFER_SIZE - 12) % IC_BUFFER_SIZE;
    ...
    MotorReadRPS = 1.0/((sumdiff*64)/10000000.0);
    return MotorReadRPS;
}
/* USER CODE END 4 */
